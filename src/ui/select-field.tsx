    ...
    <div className="relative flex flex-col gap-1">
      ...
      >
        ...
        <Select.Trigger
          ...
          className={`type-body-md bg-surface-default text-content-primary data-placeholder:text-content-muted inline-flex h-8 w-full items-center justify-between rounded-md border px-2 text-left outline-none focus:border-[#A5A5A5]/40 ${error ? "border-content-primary" : "border-line-default"}`}
        >
              ...
              <Select.List>
                {options.map((option) => (
                  <Select.Item
                    key={option.value}
                    value={option.value}
                    className="type-body-sm text-content-secondary data-highlighted:bg-fill-strong data-selected:text-content-primary relative flex h-7 w-full items-center rounded-xs px-1.5 pl-7 text-left outline-none"
                  >
